...
# run a pre-trained DGaze model for a single input data.
...
modelPath = './checkpoint/DGaze_HeadObject/checkpoint_epoch_030.tar'
...
def main():
	# Create the model
	print('\n==> Creating the model...')
	# We only utilize head and object data as input features because saliency features are difficult to obtain in real time. The experimental results in our paper validate that using only head and object data can achieve good results.
	model = DGaze_HeadObject(seqLength, seqFeatureNum, n_output, dropout_rate)

...
def main(args):
    ...
    # regress 2 values
    n_output = 2
    model = DGaze(args.seqLength, args.seqFeatureNum, args.saliencyWidth, args.saliencyNum, n_output, args.dropout_rate)
    #model = DGaze_HeadObject(args.seqLength, args.seqFeatureNum, n_output, args.dropout_rate)
    ...
    if os.path.isdir(args.checkpoint):
        ...
        if checkpointNum:
            ...
            for name in checkpoints:
                ...
                avg_time = (endTime - startTime).seconds * 1000/testSize
                print('\nAverage prediction time: {:.3f} ms'.format(avg_time))
                ...
                summaryWriter.add_scalar('Prediction Mean Error',meanPrdError, epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--featureNum', default=1702, type=int,
                        help='the number of input features (default: 1702)')    
    # the parameters for the net.
    parser.add_argument('--seqLength', default=50, type=int,
                        help=' the length of the headobject sequence (default: 50)')        
    parser.add_argument('--seqFeatureNum', default=11, type=int,
                        help=' the number of features in the headobject sequence (default: 11)')        
    ...
    parser.add_argument('-d', '--datasetDir', default = '../../DGazeDataset/dataset/DGaze/', type = str, 
                        help = 'the directory that saves the dataset (default: ../../DGazeDataset/dataset/DGaze/)')
    ...
    parser.add_argument('-c', '--checkpoint', default = '../checkpoint/DGaze/', type = str, 
                        help = 'path to save checkpoint (default: ../checkpoint/DGaze/)')
    # path to save the summary
    parser.add_argument('-s', '--summaryDir', default = '../summary/DGaze/', type = str, 
                        help = 'path to save the summary (default: ../summary/DGaze/)')
    ...
    parser.add_argument('-p', '--predictionDir', default = '../predictions/DGaze/', type = str, 
                        help = 'the directory that saves the prediction results (default: ../predictions/DGaze/)')
